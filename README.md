...
In the modeling phase, I implemented 3 machine learning algorithms,teractive process, starting with a baseline model Logistics model all the way to a Random Forest Ensemble model. The steps are as follows:

1. **Data Preprocessing** 
    + Cleaning the data, handling missing values, removing outliers and feature engineering. Feature engineering include dropping the columns like 'area code' and 'phonenumber'.
    + One-Hot-Encoding categorical columns - 'InternationalPlan','VoiceMailPlan','State'
...
6. **Hyperparameter Tuning**: 
    + To optimize the Logistic model i.e. chosing the best performing hyperparameters, I used GridSearchCV. All metrics improved after hyperparameter tuning, which suggests that the tuning process successfully enhanced the model's performance.The improvements are consistent across both training and testing datasets, which is a positive sign that the model's enhancements are not just resticted to training data. Testing accruacy increased as compared to the non-tuned SMOTE model to 0.83 from 0.57  and test AUC increased to 0.83 from 0.81.
    + To optimize the Decision Tree model, I used RandomizedSearchCV. Training a new model with the best parameters improved training accuracy in comparison to the baseline Decision Tree Model to 0.91 from 0.87, and test AUC to 0.82 FROM 0.79.  
...
|:------------------|----------------------:|-----------------------------:|-------------------------------:|
| Training Accuracy |                  0.83 |                         0.95 |                           1    |
| Testing Accuracy  |                  0.83 |                         0.89 |                           0.93 |
| Train AUC         |                  0.9  |                         0.99 |                           1    |
| Test AUC          |                  0.83 |                         0.84 |                           0.86 |  

The results indicated that the Random Forest Ensemble Model has the highest Accuracy and the highest AUC.


![ROC Curve for the Random Forest Model](Receiver operating characteristic (ROC) Curve (OPTIMIZED RANDOM FOREST MODEL).png)
...
![Top 12 Feature Importances Rankings](Top 12 Feature Importance Rankings.png)
...
These findings provide valuable insights that can help SyriaTel to identify the best **'at risk'** customers and develop proactive customer retention strategies. The feature importances fiuther helps the company to identify specific customer characteristics that could potentially lead to churn. The include:
